...
# 1. Insert _execute_chrome_js
target_1 = """def _escape_applescript(text: str) -> str:
    return text.replace("\\\\", "\\\\\\\\").replace('"', '\\\\"')"""

replacement_1 = """def _escape_applescript(text: str) -> str:
    return text.replace("\\\\", "\\\\\\\\").replace('"', '\\\\"')
...
# 2. Replace _send_whatsapp_message
import ast
...
for i, line in enumerate(lines):
    if line.startswith('def _send_whatsapp_message('):
        start_idx = i
        break

if start_idx != -1:
    for i in range(start_idx + 1, len(lines)):
        if lines[i].startswith('def _send_slack_message('):
            end_idx = i
            break

if start_idx != -1 and end_idx != -1:
    whatsapp_logic = """def _send_whatsapp_message(
    contact: str,
    message: str,
...
"""
    new_lines = lines[:start_idx] + whatsapp_logic.split('\\n') + lines[end_idx:]
    text = '\\n'.join(new_lines)
